...
def test_locate_returns_the_xcom_pulled_path(tmp_path):
    ...
    result = hook.locate({"ti": ti}).path

    ti.xcom_pull.assert_called_once_with(task_ids="run_spark_job", key="return_value")
    assert result == log_file


def test_locate_raises_when_no_xcom_value_was_pushed():
    ti = MagicMock()
    ti.xcom_pull.return_value = None
    hook = XComLogSourceHook(task_id="run_spark_job")

    with pytest.raises(AirflowException, match="No XCom value"):
        hook.locate({"ti": ti}).path


def test_locate_raises_when_the_xcom_path_does_not_exist(tmp_path):
    ti = MagicMock()
...
def test_locate_uses_a_custom_xcom_key():
    ti = MagicMock()
    ti.xcom_pull.return_value = None
